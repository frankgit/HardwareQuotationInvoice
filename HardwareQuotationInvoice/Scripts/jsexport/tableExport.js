    ...
    tableExport: function (options) {
      ...
      if (defaults.type == 'csv' || defaults.type == 'tsv' || defaults.type == 'txt') {
        ...
        if (defaults.tfootSelector.length)
            CollectCsvData($(document).find('.fixed-table-footer').find(defaults.tfootSelector), 'td,th', rowlength);
        ...
        if (defaults.tfootSelector.length)
            $rows.push.apply($rows, $(document).find('.fixed-table-footer').find(defaults.tfootSelector));
        $($rows).each(function () {
          trData = "";
